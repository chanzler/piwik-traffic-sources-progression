...
class API extends \Piwik\Plugin\API {
	...
	private static function getNumbers($idSite, $minutesToMidnight, $lastMinutes, $refTime, $origin_dt, $referrerType, $update, $statTimeSlot, $lastProcessedTimeslot) {
		if ($update) {
			$sql = "SELECT COUNT(idvisit) AS number, (ceil(ceil(UNIX_TIMESTAMP(visit_first_action_time) /1200) - @timenum  + @rownum)) AS timeslot
			                ...
			                ";
			$numbers = \Piwik\Db::fetchAll($sql, array($statTimeSlot, $idSite, ($minutesToMidnight < 20) ? $minutesToMidnight : ((($statTimeSlot - $lastProcessedTimeslot) * 20) + 40), $lastMinutes * 60));
			/*echo ($statTimeSlot);
			 echo ("#");
			 echo ($idSite);
			 echo ("#");
			...
			foreach ($numbers as &$value) {
				if ($index > 0 || $minutesToMidnight < 20) {
					...
					\Piwik\Db::query($insert, array($value['number'], $idSite, $referrerType, $value['timeslot'], $origin_dt -> format('d.m.Y')));
				}
				$index++;
			}
			for ($i = 1; $i < $statTimeSlot; $i++) {
				...
				\Piwik\Db::query($update, array($idSite, $referrerType, $i, $origin_dt -> format('d.m.Y')));
			}
		}
		$sql = "SELECT *
                ...
                ";
		$numbers = \Piwik\Db::fetchAll($sql, array($idSite, $origin_dt -> format('d.m.Y')));
		if (count($numbers) == 0) {
			...
			$sql = "SELECT *
	                	...
	                	ORDER BY timeslot ASC
	                ";
			$numbers = \Piwik\Db::fetchAll($sql, array($idSite, $clone -> format('d.m.Y')));
		}
		return $numbers;
	}
	...
	public static function getTrafficSourcesProgression($idSite, $lastMinutes = 20) {
		...
		$lastProcessedTimeslotSql = "SELECT MIN(timeslot)
				...
				AND processed = 0
				";
		$lastProcessedTimeslot = \Piwik\Db::fetchOne($lastProcessedTimeslotSql, array($idSite, $origin_dt -> format('d.m.Y')));
		...
		$socialSql = "SELECT referer_url, (ceil(ceil(UNIX_TIMESTAMP(visit_first_action_time) /1200) - @timenum  + @rownum)) AS timeslot
		            ...
		            ";
		$social = \Piwik\Db::fetchAll($socialSql, array($statTimeSlot, $idSite, ($minutesToMidnight < 20) ? $minutesToMidnight : ((($statTimeSlot - $lastProcessedTimeslot) * 20) + 40)));
		for ($i = $lastProcessedTimeslot; $i <= $statTimeSlot; $i++) {
			$socialCount = 0;
			foreach ($social as &$value) {
				if (API::isSocialUrl($value['referer_url']) && $i == $value['timeslot'])
					$socialCount++;
			}
			$insert = "UPDATE " . \Piwik\Common::prefixTable("trafficsourcesprogression_sources") . "
		               SET traffic = ?, processed = 1 WHERE idsite = ? AND source_id = ? AND timeslot = ? AND date = ?";
			\Piwik\Db::query($insert, array($socialCount, $idSite, 10, $i, $origin_dt -> format('d.m.Y')));
		}
		$socialSql = "SELECT *
                FROM " . \Piwik\Common::prefixTable("trafficsourcesprogression_sources") . "
                WHERE idsite = ?
                ...
                ORDER BY timeslot ASC
                ";
		$social = \Piwik\Db::fetchAll($socialSql, array($idSite, $origin_dt -> format('d.m.Y')));
		if (count($social) == 0) {
			$clone = clone $origin_dt;
			$clone -> modify('-1 day');
			$socialSql = "SELECT *
	                ...
	                ORDER BY timeslot ASC
	                ";
			$social = \Piwik\Db::fetchAll($socialSql, array($idSite, $clone -> format('d.m.Y')));
		}
		...
		$historicalCampaign = API::getNumbers($idSite, $minutesToMidnight, $lastMinutes, $refTime, $historical_dt, Common::REFERRER_TYPE_CAMPAIGN, false, $statTimeSlot, $lastProcessedTimeslot);
		$historicalDirect = API::getNumbers($idSite, $minutesToMidnight, $lastMinutes, $refTime, $historical_dt, Common::REFERRER_TYPE_DIRECT_ENTRY, false, $statTimeSlot, $lastProcessedTimeslot);
		$historicalSearch = API::getNumbers($idSite, $minutesToMidnight, $lastMinutes, $refTime, $historical_dt, Common::REFERRER_TYPE_SEARCH_ENGINE, false, $statTimeSlot, $lastProcessedTimeslot);
		$historicalWebsite = API::getNumbers($idSite, $minutesToMidnight, $lastMinutes, $refTime, $historical_dt, Common::REFERRER_TYPE_WEBSITE, false, $statTimeSlot, $lastProcessedTimeslot);
		$socialSql = "SELECT *
                FROM " . \Piwik\Common::prefixTable("trafficsourcesprogression_sources") . "
                WHERE idsite = ?
                AND source_id = 10
                ...
                ORDER BY timeslot ASC
                ";
		$social = \Piwik\Db::fetchAll($socialSql, array($idSite, $historical_dt -> format('d.m.Y')));
		if (count($social) == 0) {
	...
	public static function getSites() {
		...
		if (!empty($sites)) {
			foreach ($sites as $site) {
				$idSites[] = array("id" => $site['idsite'], "name" => $site['name']);
			}
		}
		return $idSites;
	}
